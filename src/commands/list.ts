...
export async function listCommand(boxManager: BoxManager, options: ListOptions): Promise<void> {
  ...
  try {
    ...
    console.log(chalk.gray('Use "qraft info <box-name>" to see detailed information about a box.'));
    ...
    if (error instanceof Error && error.message.includes('Authentication failed')) {
      ...
      console.error(chalk.cyan(`  qraft auth token --registry ${options.registry || 'default'} <your-token>`));
