...
export async function listCommand(boxManager: BoxManager, options: ListOptions): Promise<void> {
  ...
  try {
    ...
    console.log(chalk.gray('\nUse "unbox copy <box-name>" to copy a box to your project.'));
    console.log(chalk.gray('Use "unbox info <box-name>" to see detailed information about a box.'));
    ...
    if (error instanceof Error && error.message.includes('Authentication failed')) {
      ...
      console.error(chalk.cyan(`  unbox auth token --registry ${options.registry || 'default'} <your-token>`));
