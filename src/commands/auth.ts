    ...
    .action(async (options) => {
      try {
        if (!options.registry) {
          ...
          console.error(chalk.gray('Use: qraft auth token --registry <name> --token <token>'));
          process.exit(1);
        }
      ...
      try {
        if (options.all) {
          ...
          if (!hasAuth) {
            ...
            console.log(chalk.cyan(`  qraft auth token --registry ${options.registry}`));
            return;
          }
          ...
          if (!hasAuth) {
            ...
            console.log(chalk.gray('Set up authentication:'));
            console.log(chalk.cyan('  qraft auth login'));
            return;
          }
